...
public interface AprilTagVisionIO {
	...
	public static class AprilTagVisionIOInputs {
		...
		public Pose3d[] visionPoses = List.of(new Pose3d(), new Pose3d(), new Pose3d(), new Pose3d()).toArray(new Pose3d[0]); //
	 ...
	 */
	default Matrix<N3, N1> getEstimationStdDevs(EstimatedRobotPose estimatedPose, CameraResolution resolution) {
		var estStdDevs =
			switch (resolution) {
		...
		for (var tgt : targets) {
			...
			numTags++;
			avgDist += tagPose.get().toPose2d().minus(estimatedPose.estimatedPose.toPose2d()).getTranslation().getNorm();
		}
	 ...
	 */
	default void getEstimatedPoseUpdates(CameraPoseEstimator[] poseEstimators, Pose3d[] poseArray, double[] timestampArray, double[] visionStdArray, double[] latencyArray) {
		for (int i = 0; i < poseEstimators.length; i++) {
			...
			pose.ifPresentOrElse(
				estimatedRobotPose -> {
					...
					Matrix<N3, N1> stdDevs = getEstimationStdDevs(estimatedRobotPose, poseEstimators[index].resolution);
					System.arraycopy(stdDevs.getData(), 0, visionStdArray, index * 3, 3);
					if (GlobalConstants.ROBOT_MODE == RobotMode.SIM) { // Don't update latency if in sim. It doesn't work for some reason. TODO
						// fix that
				...
				() -> {
					...
					poseArray[index] = new Pose3d(Double.NaN, Double.NaN, Double.NaN, new Rotation3d(Double.NaN, Double.NaN, Double.NaN));
